...
describe('getInvalidationPathsFromKeys', () => {
  it('should provide correct root paths', () => {
    ...
    expect(paths).toEqual(['/index.html', '/blog.html', '/']);
  });

  it('should provide correct preview paths', () => {
    const prefix = 'preview/branch';
    const keys = [
      'preview/branch/blog.html',
      'preview/branch/index.html',
      'preview/branch/css/styles.css',
    ];
    const paths = getInvalidationPathsFromKeys(keys, prefix);
