...
function getInputs() {
    ...
    const s3BucketName = (0,core.getInput)('s3BucketName', {
        required: true,
        trimWhitespace: true,
    });
    const s3AllowedOrigins = (0,core.getInput)('s3AllowedOrigins', {
        required: true,
        trimWhitespace: true,
    });
    const rootCloudFrontHosts = (0,core.getInput)('rootCloudFrontHosts', {
        required: true,
        trimWhitespace: true,
    });
    const previewCloudFrontHosts = (0,core.getInput)('previewCloudFrontHosts', {
        required: true,
        trimWhitespace: true,
    });
    const cacheCorsPathPattern = (0,core.getInput)('cacheCorsPathPattern', {
        required: true,
        trimWhitespace: true,
    });
    const certificateARN = (0,core.getInput)('certificateARN', {
        required: true,
        trimWhitespace: true,
    });
    const outDir = (0,core.getInput)('outDir', {
        required: true,
        trimWhitespace: true,
    });
    return {
...
let statusLogs = {};
function logging_logStatus(status) {
    if (!(status in statusLogs)) {
        statusLogs[status] = true;
        if (status === String(StackStatus.ROLLBACK_IN_PROGRESS)) {
            warning(`${StackStatus.ROLLBACK_IN_PROGRESS} detected! **Check the CloudFormation events in the AWS Console for more information.** ` +
...
function logging_resetStatusLogs() {
    statusLogs = {};
}
...
function getCloudFormationParameters(cfProjectName, s3BucketName, s3AllowedOrigins, rootCloudFrontHosts, previewCloudFrontHosts, cacheCorsPathPattern, certificateARN, lambdaVersion) {
    return [
        {
...
async function waitForStackStatus(client, cfStackName, status, delayMs = defaultProgressDelayMs) {
    try {
        ...
        debug(`Unable to wait for status ${status} because ${e.message}`);
    }
    finally {
        resetStatusLogs();
    }
...
async function deleteExistingStack(client, cfStackName) {
    ...
    await waitForStackStatus(client, cfStackName, String(StackStatus.DELETE_COMPLETE));
...
async function createOrUpdateStack(cfStackName, parameters) {
    ...
    debug(`Using parameters: ${parameters
    ...
    if (hasExistingStack) {
        ...
        if (shouldDelete) {
            warning(`Deleting existing stack ${cfStackName}, due to ${StackStatus.ROLLBACK_COMPLETE} status`);
...
async function run() {
    try {
        ...
        const cfParameters = getCloudFormationParameters(inputs.cfStackName, inputs.s3BucketName, inputs.s3AllowedOrigins, inputs.rootCloudFrontHosts, inputs.previewCloudFrontHosts, inputs.cacheCorsPathPattern, inputs.certificateARN, lambdaVersion);
